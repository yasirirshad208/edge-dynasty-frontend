...
const Navbar = () => {
      ...
      if (token) {
        try {
          const response = await axios.get(
            "https://api.edgedynasty.com/api/user/verify/admin",
            {
              headers: {
  ...
  const handleSearchInput = async (e) => {
    ...
    if (query.length > 2) { // Minimum 3 characters to search
      try {
        const response = await axios.get(`https://api.edgedynasty.com/api/product/search?searchQuery=${query}`);
              ...
              <div className="absolute bg-white w-full shadow-lg rounded-lg mt-2 z-50">
                {searchResults.map((product) => (
                  ...
                  >
                    {/* Product Image */}
                    <img
                      src={`https://api.edgedynasty.com/${product.mainImage}`}
                      alt={product.name}
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item._id} className="flex items-center my-6">
                <img
                  src={`https://api.edgedynasty.com/${item.mainImage}`}
                  alt={item.name}
