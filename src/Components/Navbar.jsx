...
const Navbar = () => {
      ...
      if (token) {
        try {
          const response = await axios.get(
            "https://api.edgedynasty.com:5000/api/user/verify/admin",
            {
              headers: {
  ...
  const handleSearchInput = async (e) => {
    ...
    if (query.length > 2) { // Minimum 3 characters to search
      try {
        const response = await axios.get(`https://api.edgedynasty.com:5000/api/product/search?searchQuery=${query}`);
      ...
      <div
        ...
        >
          <div className="paraorg logo-slide flex justify-between  text-white text-[14px] font-[700] p-[10] topSlider">
            <span className="slide-name text-[20px]"><i class="fa-solid fa-bolt-lightning"></i> Flat 30% offer</span>
            <span className="slide-name text-[20px]"><i class="fa-solid fa-bolt-lightning"></i> Flat 30% offer</span>
            <span className="slide-name text-[20px]"><i class="fa-solid fa-bolt-lightning"></i> Flat 30% offer</span>
            <span className="slide-name text-[20px]"><i class="fa-solid fa-bolt-lightning"></i> Flat 30% offer</span>
            <span className="slide-name text-[20px]"><i class="fa-solid fa-bolt-lightning"></i> Flat 30% offer</span>
              ...
              <div className="absolute bg-white w-full shadow-lg rounded-lg mt-2 z-50">
                {searchResults.map((product) => (
                  ...
                  >
                    {/* Product Image */}
                    <img
                      src={`https://api.edgedynasty.com:5000/${product.mainImage}`}
                      alt={product.name}
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item._id} className="flex items-center my-6">
                <img
                  src={`https://api.edgedynasty.com:5000/${item.mainImage}`}
                  alt={item.name}
