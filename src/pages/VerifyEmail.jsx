...
let response;
      try {
        if(type=== 'signup'){
         response = await axios.get(`http://3.87.157.93:5000/api/user/verify-email?token=${token}&type=${type}`);
        }else if(type=== 'forgot-password'){
           response = await axios.get(`http://3.87.157.93:5000/api/user/verify-reset-email?token=${token}&type=${type}`);
        }
